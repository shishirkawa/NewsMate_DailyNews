...
from newsmate.config.config import NewsMateConfig
...
class GeneratePDF:
    def __init__(self, date):
        ...
        self.output_dir = NewsMateConfig.OUTPUT_DIR
        os.makedirs(f"{self.output_dir}/{self.date_obj.strftime('%d-%m-%Y')}", exist_ok=True)
        self.filename = f"{self.output_dir}/{self.date_obj.strftime('%d-%m-%Y')}/NewsMate {self.date_obj.strftime('%d-%m-%Y')}.pdf"
        self.story = []
    ...
    def save_pdf(self):
        try:
            ...
            doc.title = f"NewsMate | {self.date_obj.strftime('%d %B %Y')}"
            doc.author = "Shishir Kawa"
            doc.subject = f"News for {self.date_obj.strftime('%d %B %Y')}"
            doc.creator = "NewsMate"
